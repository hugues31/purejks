...
class AbstractTest(unittest.TestCase):
    def find_private_key(self, ks, alias):
        ...
        if not isinstance(pk, jks.PrivateKeyEntry):
            self.fail(f"Private key entry not found: {alias}")
    ...
    def find_secret_key(self, ks, alias):
        ...
        if not isinstance(sk, jks.SecretKeyEntry):
            self.fail(f"Secret key entry not found: {alias}")
    ...
    def find_cert(self, ks, alias):
        ...
        if not isinstance(c, jks.TrustedCertEntry):
            self.fail(f"Certificate entry not found: {alias}")

        self.assertTrue(isinstance(c.cert, bytes))
        self.assertTrue(isinstance(c.type, str))
        return c
...
class JksTests(AbstractTest):
    ...
    def test_bad_keystore_format(self):
        self.assertRaises(
            jks.util.BadKeystoreFormatException, jks.KeyStore.loads, b"\x00\x00\x00\x00", ""
        )  # bad magic bytes
        self.assertRaises(
            jks.util.BadKeystoreFormatException, jks.KeyStore.loads, b"\xfe\xed\xfe\xed\x00", ""
        )  # insufficient store version bytes
        self.assertRaises(
            jks.util.UnsupportedKeystoreVersionException, jks.KeyStore.loads, b"\xfe\xed\xfe\xed\x00\x00\x00\x00", ""
        )  # unknown store version
        self.assertRaises(
            jks.util.KeystoreSignatureException,
            jks.KeyStore.loads,
            b"\xfe\xed\xfe\xed\x00\x00\x00\x02\x00\x00\x00\x00" + b"\x00" * 20,
            "",
        )  # bad signature
        self.assertRaises(
            jks.util.BadKeystoreFormatException,
            jks.KeyStore.loads,
    ...
    def test_trailing_data(self):
        ...
        with open(KS_PATH + "/jks/RSA1024.jks", "rb") as f:
            store_bytes = f.read()
        jks.KeyStore.loads(store_bytes + b"\x00" * 1, "12345678")
        jks.KeyStore.loads(store_bytes + b"\x00" * 1000, "12345678")

    def test_rsa_1024(self):
        store = jks.KeyStore.load(KS_PATH + "/jks/RSA1024.jks", "12345678")
        pk = self.find_private_key(store, "mykey")
        self.assertEqual(store.store_type, "jks")
        self.check_pkey_and_certs_equal(
            pk, jks.util.RSA_ENCRYPTION_OID, expected.RSA1024.private_key, expected.RSA1024.certs
        )
    ...
    def test_rsa_2048_3certs(self):
        ...
        self.assertEqual(store.store_type, "jks")
        self.check_pkey_and_certs_equal(
            pk, jks.util.RSA_ENCRYPTION_OID, expected.RSA2048_3certs.private_key, expected.RSA2048_3certs.certs
        )
    ...
    def test_custom_entry_passwords(self):
        ...
        self.check_pkey_and_certs_equal(
            ...
            expected.custom_entry_passwords.private_key,
    ...
    def test_duplicate_aliases(self):
        self.assertRaises(
            jks.DuplicateAliasException, jks.KeyStore.load, KS_PATH + "/jks/duplicate_aliases.jks", "12345678"
        )

    def test_non_ascii_jks_password(self):
        store = jks.KeyStore.load(KS_PATH + "/jks/non_ascii_password.jks", "\u10da\u0028\u0ca0\u76ca\u0ca0\u10da\u0029")
        pk = self.find_private_key(store, "mykey")
        self.assertEqual(store.store_type, "jks")
        self.check_pkey_and_certs_equal(
            pk,
            jks.util.RSA_ENCRYPTION_OID,
            expected.jks_non_ascii_password.private_key,
    ...
    def test_load_and_save_keystore_non_ascii_password(self):
        with open(KS_PATH + "/jks/non_ascii_password.jks", "rb") as file:
            keystore_bytes = file.read()
        store = jks.KeyStore.loads(keystore_bytes, "\u10da\u0028\u0ca0\u76ca\u0ca0\u10da\u0029", False)
        resaved_keystore_bytes = store.saves("\u10da\u0028\u0ca0\u76ca\u0ca0\u10da\u0029")
        # since we didn't decrypt the key, the keystores should be identical
        self.assertEqual(keystore_bytes, resaved_keystore_bytes)

    def test_create_and_load_keystore_non_ascii_password(self):
        cert = jks.PrivateKeyEntry.new(
            "mykey", expected.jks_non_ascii_password.certs, expected.jks_non_ascii_password.private_key
        )
        store = jks.KeyStore.new("jks", [cert])
        store_bytes = store.saves("\u10da\u0028\u0ca0\u76ca\u0ca0\u10da\u0029")
        store2 = jks.KeyStore.loads(store_bytes, "\u10da\u0028\u0ca0\u76ca\u0ca0\u10da\u0029")
    ...
    def test_create_and_load_non_ascii_alias(self):
        cert = jks.PrivateKeyEntry.new(
            "\xe6\xe6\xe6\xf8\xf8\xf8\xe5\xe5\xf8\xe6", expected.RSA1024.certs, expected.RSA1024.private_key
        )
        store = jks.KeyStore.new("jks", [cert])
        store_bytes = store.saves("12345678")
        store2 = jks.KeyStore.loads(store_bytes, "12345678")
        pk = self.find_private_key(store2, "\xe6\xe6\xe6\xf8\xf8\xf8\xe5\xe5\xf8\xe6")
        self.assertEqual(store2.store_type, "jks")
        self.check_pkey_and_certs_equal(
            pk, jks.util.RSA_ENCRYPTION_OID, expected.RSA1024.private_key, expected.RSA1024.certs
        )
    ...
    def test_create_and_load_custom_entry_passwords(self):
        cert = jks.PrivateKeyEntry.new(
            "mykey", expected.custom_entry_passwords.certs, expected.custom_entry_passwords.private_key
        )
        store = jks.KeyStore.new("jks", [cert])
        pk = self.find_private_key(store, "mykey")
        self.assertTrue(pk.is_decrypted())
    ...
    def test_create_and_load_keystore_raw_rsa(self):
        cert = jks.PrivateKeyEntry.new(
            "mykey", expected.RSA2048_3certs.certs, expected.RSA2048_3certs.raw_private_key, "rsa_raw"
        )
        store = jks.KeyStore.new("jks", [cert])
        store_bytes = store.saves("12345678")
        store2 = jks.KeyStore.loads(store_bytes, "12345678")
    ...
    def test_create_and_load_keystore_trusted_certs(self):
        cert1 = jks.TrustedCertEntry(
            type="X.509", cert=expected.RSA2048_3certs.certs[0], timestamp=1463338684456, alias="cert1"
        )
        cert2 = jks.TrustedCertEntry(
            type="X.509", cert=expected.RSA2048_3certs.certs[1], timestamp=1463338684456, alias="cert2"
        )
        cert3 = jks.TrustedCertEntry(
            type="X.509", cert=expected.RSA2048_3certs.certs[2], timestamp=1463338684456, alias="cert3"
        )
    ...
    def test_create_and_load_keystore_both_trusted_and_private(self):
        ...
        self.assertEqual(store.store_type, "jks")
        self.check_pkey_and_certs_equal(
            pk2, jks.util.RSA_ENCRYPTION_OID, expected.RSA2048_3certs.private_key, expected.RSA2048_3certs.certs
        )
    ...
    def test_create_unknown_key_format(self):
        self.assertRaises(
            jks.util.UnsupportedKeyFormatException, jks.PrivateKeyEntry.new, "alias", "cert", "key", "ecdsa"
        )
...
class JceTests(AbstractTest):
    ...
    def test_bad_keystore_format(self):
        ...
        self.assertRaises(
            jks.util.BadKeystoreFormatException, jks.KeyStore.loads, b"\xce\xce\xce\xce\x00", ""
        )  # insufficient store version bytes
        self.assertRaises(
            jks.util.UnsupportedKeystoreVersionException, jks.KeyStore.loads, b"\xce\xce\xce\xce\x00\x00\x00\x00", ""
        )  # unknown store version
        self.assertRaises(
            jks.util.KeystoreSignatureException,
            jks.KeyStore.loads,
            b"\xce\xce\xce\xce\x00\x00\x00\x02\x00\x00\x00\x00" + b"\x00" * 20,
            "",
        )  # bad signature
        self.assertRaises(
            jks.util.BadKeystoreFormatException,
    ...
    def test_custom_entry_passwords(self):
        store = jks.KeyStore.load(
            KS_PATH + "/jceks/custom_entry_passwords.jceks", "store_password"
        )  # shouldn't throw, we're not yet trying to decrypt anything at this point
        self.assertEqual(store.store_type, "jceks")
    ...
    def test_duplicate_aliases(self):
        self.assertRaises(
            jks.DuplicateAliasException, jks.KeyStore.load, KS_PATH + "/jceks/duplicate_aliases.jceks", "12345678"
        )
...
class JceOnlyTests(AbstractTest):
    ...
    def test_unknown_type_of_sealed_object(self):
        ...
        self.assertRaises(
            jks.UnexpectedJavaTypeException,
            lambda: jks.KeyStore.load(KS_PATH + "/jceks/unknown_type_of_sealed_object.jceks", "12345678"),
        )
    ...
    def test_unknown_type_inside_sealed_object(self):
        ...
        self.assertRaises(
            jks.UnexpectedJavaTypeException,
            lambda: jks.KeyStore.load(KS_PATH + "/jceks/unknown_type_inside_sealed_object.jceks", "12345678"),
        )

    def test_unknown_sealed_object_sealAlg(self):
        self.assertRaises(
            jks.UnexpectedAlgorithmException,
            lambda: jks.KeyStore.load(KS_PATH + "/jceks/unknown_sealed_object_sealAlg.jceks", "12345678"),
        )
...
class BksOnlyTests(AbstractTest):
    def check_bks_entry(self, entry, store_type):
        ...
        self.assertTrue(isinstance(entry.alias, str))
    ...
    def check_cert_entry(self, entry, store_type):
        ...
        self.assertTrue(isinstance(entry.type, str))
    ...
    def check_plain_key_entry(self, key_entry, store_type, check_type=True):
        ...
        self.assertTrue(isinstance(key_entry.format, str))
        self.assertTrue(isinstance(key_entry.algorithm, str))
        ...
        if key_entry.type == jks.bks.KEY_TYPE_PRIVATE:
            ...
            self.fail(f"No such key type: {repr(key_entry.type)}")
    ...
    def test_bad_bks_keystore_format(self):
        self.assertRaises(
            jks.util.BadKeystoreFormatException, jks.bks.BksKeyStore.loads, b"\x00\x00\x00", ""
        )  # insufficient store version bytes
        self.assertRaises(
            jks.util.UnsupportedKeystoreVersionException,
            jks.bks.BksKeyStore.loads,
            b"\x00\x00\x00\x00" + b"\x00\x00\x00\x08" + (b"\xff" * 8) + b"\x00\x00\x00\x14" + b"\x00" + (b"\x00" * 20),
            "",
        )  # unknown store version
        self.assertRaises(
            jks.util.KeystoreSignatureException,
            jks.bks.BksKeyStore.loads,
            b"\x00\x00\x00\x02" + b"\x00\x00\x00\x08" + (b"\xff" * 8) + b"\x00\x00\x00\x14" + b"\x00" + (b"\x00" * 20),
            "",
        )  # bad HMAC
        self.assertRaises(
            jks.util.BadKeystoreFormatException,
            jks.bks.BksKeyStore.loads,
            b"\x00\x00\x00\x02" + b"\x00\x00\x00\x08" + (b"\xff" * 8) + b"\x00\x00\x00\x14" + b"\x00" + (b"\x00" * 19),
            "",
        )  # insufficient HMAC bytes

    def test_bad_uber_keystore_format(self):
        self.assertRaises(
            jks.util.BadKeystoreFormatException, jks.bks.UberKeyStore.loads, b"\x00\x00\x00", ""
        )  # insufficient store version bytes
        self.assertRaises(
            jks.util.UnsupportedKeystoreVersionException,
            jks.bks.UberKeyStore.loads,
        ...
        self.assertRaises(
            ...
            password,
        )  # empty embedded BKS entries + bad SHA-1 hash of that 0-byte store
    ...
    def _test_christmas_store(self, store, store_type):
        ...
        self.check_pkey_and_certs_equal(
            sealed_private,
            jks.util.RSA_ENCRYPTION_OID,
            expected.bks_christmas.private_key,
        ...
        self.check_secret_key_equal(
            sealed_secret, "AES", 128, b"\x3f\x68\x05\x04\xc6\x6c\xc2\x5a\xae\x65\xd0\xfa\x49\xc5\x26\xec"
        )
    ...
    def _test_custom_entry_passwords(self, store, store_type):
        ...
        for a in attrs_encrypted_public:
            self.assertRaises(jks.util.NotYetDecryptedException, getattr, sealed_public, a)
        for a in attrs_non_encrypted:
            getattr(sealed_public, a)  # shouldn't throw
        ...
        for a in attrs_encrypted_private:
            self.assertRaises(jks.util.NotYetDecryptedException, getattr, sealed_private, a)
        for a in attrs_non_encrypted:
            getattr(sealed_private, a)  # shouldn't throw
        ...
        for a in attrs_encrypted_secret:
            self.assertRaises(jks.util.NotYetDecryptedException, getattr, sealed_secret, a)
        for a in attrs_non_encrypted:
            getattr(sealed_secret, a)  # shouldn't throw
    ...
    def test_trailing_data_uber(self):
        ...
        with open(KS_PATH + "/uber/christmas.uber", "rb") as f:
            christmas_store_bytes = f.read()
        self.assertRaises(
            jks.util.DecryptionFailureException,
            jks.bks.UberKeyStore.loads,
            christmas_store_bytes + b"\x00" * 256,
            "12345678",
        )  # maintain multiple of 16B -> decryption failure
        self.assertRaises(
            jks.util.BadKeystoreFormatException,
            jks.bks.UberKeyStore.loads,
            christmas_store_bytes + b"\x00" * 255,
            "12345678",
        )  # break multiple of 16B -> bad format
...
class MiscTests(AbstractTest):
    ...
    def test_strip_pkcs5_padding(self):
        ...
        self.assertEqual(
            jks.util.strip_pkcs5_padding(b"\x01\x02\x03\x04\x05\x06\x07\x01"), b"\x01\x02\x03\x04\x05\x06\x07"
        )
        ...
        self.assertRaises(
            jks.util.BadPaddingException, jks.util.strip_pkcs5_padding, b"\x00\x00\x00\x00\x00\x00\x00\x00"
        )

    def test_sun_jce_pbe_decrypt(self):
        self.assertEqual(
            b"sample",
            jks.sun_crypto.jce_pbe_decrypt(
                b"\xc4\x20\x59\xac\x54\x03\xc7\xbf", "my_password", b"\x01\x02\x03\x04\x05\x06\x07\x08", 42
            ),
        )
        self.assertEqual(
            b"sample",
            jks.sun_crypto.jce_pbe_decrypt(
                b"\xef\x9f\xbd\xc5\x91\x5f\x49\x50", "my_password", b"\x01\x02\x03\x04\x01\x02\x03\x05", 42
            ),
        )
        self.assertEqual(
            b"sample",
            jks.sun_crypto.jce_pbe_decrypt(
                b"\x72\x8f\xd8\xcc\x21\x41\x25\x80", "my_password", b"\x01\x02\x03\x04\x01\x02\x03\x04", 42
            ),
        )

    def test_pkcs12_key_derivation(self):
        self.assertEqual(
            jks.rfc7292.derive_key(
                hashlib.sha1, jks.rfc7292.PURPOSE_MAC_MATERIAL, "", b"\x01\x02\x03\x04\x05\x06\x07\x08", 1000, 16
            ),
            b"\xe7\x76\x85\x01\x6a\x53\x62\x1e\x9a\x2a\x8a\x0f\x80\x00\x2e\x70",
        )
        self.assertEqual(
            jks.rfc7292.derive_key(
                hashlib.sha1, jks.rfc7292.PURPOSE_MAC_MATERIAL, "", b"\x01\x02\x03\x04\x05\x06\x07\x08", 1000, 17
            ),
        ...
        self.assertEqual(
            jks.rfc7292.derive_key(
                hashlib.sha1,
                jks.rfc7292.PURPOSE_KEY_MATERIAL,
                "",
        ...
        self.assertEqual(
            jks.rfc7292.derive_key(
                hashlib.sha1, jks.rfc7292.PURPOSE_MAC_MATERIAL, "password", b"\x01\x02\x03\x04\x05\x06\x07\x08", 1000, 0
            ),
            b"",
        )
        self.assertEqual(
            jks.rfc7292.derive_key(
                hashlib.sha1,
                jks.rfc7292.PURPOSE_MAC_MATERIAL,
                "password",
        ...
        self.assertEqual(
            jks.rfc7292.derive_key(
                hashlib.sha1, jks.rfc7292.PURPOSE_IV_MATERIAL, "password", b"\x01\x02\x03\x04\x05\x06\x07\x08", 1000, 17
            ),
            b"\x27\x68\x91\x7c\xf9\xf4\x33\xb0\xa6\x4a\x9f\xcc\xbc\x80\x5f\xd6\x48",
        )

        fancy_password = "\u10da\u0028\u0ca0\u76ca\u0ca0\u10da\u0029"
        self.assertEqual(
            jks.rfc7292.derive_key(
                hashlib.sha1,
                jks.rfc7292.PURPOSE_KEY_MATERIAL,
    ...
    def test_decrypt_PBEWithSHAAnd3KeyTripleDESCBC(self):
        ...
        self.assertEqual(
            b"sample",
            jks.rfc7292.decrypt_PBEWithSHAAnd3KeyTripleDESCBC(
                b"\x69\xea\xff\x28\x65\x85\x0a\x68", "mypassword", b"\x01\x02\x03\x04\x05\x06\x07\x08", 1000
            ),
        )
        self.assertEqual(
            b"sample",
            jks.rfc7292.decrypt_PBEWithSHAAnd3KeyTripleDESCBC(
                b"\x73\xf1\xc7\x14\x74\xa3\x04\x59", fancy_password, b"\x01\x02\x03\x04\x05\x06\x07\x08", 1000
            ),
        )
        ...
        self.assertRaises(
            jks.util.BadDataLengthException, jks.rfc7292.decrypt_PBEWithSHAAnd3KeyTripleDESCBC, b"\x00", "", b"", 20
        )
    ...
    def test_decrypt_PBEWithSHAAndTwofishCBC(self):
        ...
        self.assertEqual(
            b"sample",
            jks.rfc7292.decrypt_PBEWithSHAAndTwofishCBC(
                b"\xc5\x22\x81\xc9\xa2\x24\x4b\x10\xf9\x1c\x6c\xbc\x67\x10\x42\x3e",
                "mypassword",
                b"\x01\x02\x03\x04\x05\x06\x07\x08",
                1000,
            ),
        ...
        self.assertEqual(
            b"sample",
            jks.rfc7292.decrypt_PBEWithSHAAndTwofishCBC(
                b"\xc8\xc4\x7a\xe6\xa7\xc2\x80\xd7\x05\x5f\xe2\x4f\xf4\x20\x30\x7c",
                fancy_password,
                b"\x01\x02\x03\x04\x05\x06\x07\x08",
                1000,
            ),
        ...
        self.assertRaises(
            jks.util.BadDataLengthException, jks.rfc7292.decrypt_PBEWithSHAAndTwofishCBC, b"\x00", "", b"", 20
        )
