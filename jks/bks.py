...
from pyasn1_modules import rfc2459, rfc5208
...
class AbstractBksEntry(AbstractKeystoreEntry):
    ...
    def __init__(self, **kwargs):
        ...
        self._encrypted: Any = kwargs.get("encrypted")
    ...
    def __init__(self, type, format, algorithm, encoded, **kwargs):
        ...
        if self.type == KEY_TYPE_PRIVATE:
            if self.format not in ["PKCS8", "PKCS#8"]:
                raise UnexpectedKeyEncodingException(f"Unexpected encoding for private key entry: '{self.format}'")
            ...
            if self.format not in ["X.509", "X509"]:
                raise UnexpectedKeyEncodingException(f"Unexpected encoding for public key entry: '{self.format}'")
            ...
            if self.format != "RAW":
                raise UnexpectedKeyEncodingException(f"Unexpected encoding for raw key entry: '{self.format}'")
            # self.encoded is an unwrapped/raw cryptographic key
            self.key = encoded
            self.key_size = len(encoded) * 8

        else:
            raise UnexpectedKeyEncodingException(f"Key format '{self.format}' not recognized")
...
class BksSealedKeyEntry(AbstractBksEntry):
    ...
    def __getattr__(self, name):
        if not self.is_decrypted():
            raise NotYetDecryptedException(
                f"Cannot access attribute '{name}'; entry not yet decrypted, call decrypt() with the correct password first"
            )
    ...
    def decrypt(self, key_password):
        ...
        #
        try:
            decrypted = rfc7292.decrypt_PBEWithSHAAnd3KeyTripleDESCBC(
                encrypted_blob, key_password, salt, iteration_count
            )
        except BadDataLengthException as e:
            raise BadKeystoreFormatException(f"Bad BKS entry format: {e}") from e
        except BadPaddingException as e:
            raise DecryptionFailureException(f"Failed to decrypt data for key '{self.alias}'; wrong password?") from e
...
class BksKeyStore(AbstractKeystore):
    ...
    def __init__(self, store_type, entries, version=2):
        super().__init__(store_type, entries)
    ...
    def certs(self):
        ...
        return dict([(a, e) for a, e in self.entries.items() if isinstance(e, BksTrustedCertEntry)])

    @property
    def secret_keys(self):
        """A subset of the :attr:`entries` dictionary, filtered down to only
        those entries of type :class:`BksSecretKeyEntry`."""
        return dict([(a, e) for a, e in self.entries.items() if isinstance(e, BksSecretKeyEntry)])
        ...
        try:
            ...
            if version not in [1, 2]:
                raise UnsupportedKeystoreVersionException(
                    "Unsupported BKS keystore version; only V1 and V2 supported, found v" + repr(version)
                )
            ...
            store_type = "bks"
            entries, size = cls._load_bks_entries(
                data[pos:], store_type, store_password, try_decrypt_keys=try_decrypt_keys
            )
            ...
            hmac_key = rfc7292.derive_key(
                hmac_fn, rfc7292.PURPOSE_MAC_MATERIAL, store_password, salt, iteration_count, hmac_key_size // 8
            )
            ...
            if len(store_hmac) != hmac_digest_size:
                raise BadKeystoreFormatException(
                    f"Bad HMAC size; found {len(store_hmac)} bytes, expected {hmac_digest_size} bytes"
                )
            ...
            raise BadKeystoreFormatException(e) from e
    ...
    def _load_bks_entries(cls, data, store_type, store_password, try_decrypt_keys=False):
        ...
        while pos < len(data):
            ...
            if _type == 1:  # certificate
                ...
                # key: plaintext key entry, i.e. same as sealed key but without the PBEWithSHAAnd3KeyTripleDESCBC layer
                entry, pos = cls._read_bks_key(data, pos, store_type)
            elif _type == 3:
                # secret key: opaque arbitrary data blob, stored as-is by the keystore; can be anything
                # (assumed to already be protected when supplied).
                entry, pos = cls._read_bks_secret(data, pos, store_type)
            elif _type == 4:
                # sealed key; a well-formatted certificate, private key or public key, encrypted by the BKS
                # implementation with a standard algorithm at save time
                entry, pos = cls._read_bks_sealed(data, pos, store_type)
            else:
                raise BadKeystoreFormatException(f"Unexpected keystore entry type {_type}")
            ...
            if alias in entries:
                raise DuplicateAliasException(f"Found duplicate alias '{alias}'")
            entries[alias] = entry
...
class UberKeyStore(BksKeyStore):
    ...
    def loads(cls, data, store_password, try_decrypt_keys=True):
        ...
        try:
            ...
            try:
                decrypted = rfc7292.decrypt_PBEWithSHAAndTwofishCBC(
                    encrypted_bks_store, store_password, salt, iteration_count
                )
            except BadDataLengthException as e:
                raise BadKeystoreFormatException(f"Bad UBER keystore format: {e}") from e
            except BadPaddingException as e:
                raise DecryptionFailureException("Failed to decrypt UBER keystore: bad password?") from e
            ...
            if len(bks_hash) != hash_digest_size:
                raise BadKeystoreFormatException(
                    f"Insufficient signature bytes; found {len(bks_hash)} bytes, expected {hash_digest_size} bytes"
                )
            ...
            store_type = "uber"
            entries, size = cls._load_bks_entries(
                bks_store, store_type, store_password, try_decrypt_keys=try_decrypt_keys
            )
            return cls(store_type, entries, version=version)
    ...
    def __init__(self, store_type, entries, version=1):
        super().__init__(store_type, entries, version=version)
