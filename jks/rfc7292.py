...
from .util import BadDataLengthException, add_pkcs7_padding, strip_pkcs7_padding, xor_bytearrays
...
def derive_key(hashfn, purpose_byte, password_str, salt, iteration_count, desired_key_size):
    ...
    I = S + P  # noqa: E741 (notation of RFC 7292, appendix B.2)
    c = (desired_key_size + u - 1) // u
    derived_key = b""

    for _i in range(1, c + 1):
        A = hashfn(bytes(D + I)).digest()
        for _j in range(iteration_count - 1):
            A = hashfn(A).digest()
