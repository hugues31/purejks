...
SUN_JCE_ALGO_ID = (
    ...
    19,
    1,
)  # PBE_WITH_MD5_AND_DES3_CBC_OID (non-published, modified version of PKCS#5 PBEWithMD5AndDES)
...
def _jce_pbe_derive_key_and_iv(password, salt, iteration_count):
    if len(salt) != 8:
        raise ValueError(
            f"Expected 8-byte salt for PBEWithMD5AndTripleDES (OID {'.'.join(str(i) for i in SUN_JCE_ALGO_ID)}), "
            f"found {len(salt)} bytes"
        )
    ...
    try:
        ...
        raise ValueError("Key password contains non-ASCII characters") from e
